...
- **`specter ingest --merge`.** Builds the output from the named results files alone rather than accumulating into an existing one, so a criterion that passed in an earlier run and produces no entry now does not keep a stale pass. Each input is capped at 16 MiB, and so is the merged output: two inputs that each fit can sum past the cap, and the result would be a file `coverage` cannot read. The two refusals read differently, because a missing stream declaration can be added and a size cannot. **Action:** none unless you split your test run across jobs. If you do, have each job write its own file with `--stream`, then merge them in a final step.
