...
- **Multi-stream evidence: `specter ingest --stream`.** A results file can now record which body of evidence each entry came from. `--stream <name>` labels every entry one run produces, and the file gains a top-level `streams` array carrying one row per stream with its `name`, how many test cases were `scanned`, and how many `(spec_id, ac_id)` pairs were `extracted`. The merge key becomes criterion plus stream, so two streams reporting on one criterion produce two entries rather than one. Collapsing them would destroy the distinction the block exists to record: they are two facts, not a contradiction to resolve. On the outcome-verified paths, which are `--strictness threshold` and `zero-tolerance`, a criterion is covered when at least one stream holds an entry for it and every stream holding one reports passed, so the coverage number does not move when a project splits one job into two. Under `--strictness annotation` the results file is not consulted for coverage at all, so streams change nothing there. **Action:** none. A run with no `--stream` writes no `streams` key at all and behaves exactly as it did in v0.14.

  The block is what separates a stream that ran and found nothing from one that never ran. Both leave zero entries, so a per-entry label alone cannot tell them apart. A stream that ran appears in the array with `extracted: 0`.

- **`specter ingest --merge`.** Builds the output from the named results files alone, never accumulating into an existing output. That is deliberately the opposite of the default: accumulating is right within one run and wrong across runs, where a criterion that passed before and produced no entry keeps a stale pass. It matches CI, where streams run as separate jobs and each writes its own file. Each input is capped at 16 MiB, checked by `stat` before the file is read. **Action:** none unless you split your test run across jobs. If you do, have each job write its own file with `--stream`, then merge them in a final step.

- **A silent-package count on each stream.** `zero_test_event_packages` counts packages the runner reported on that produced no test event at all. `ingest` used to discard every runner event with no test name, which threw away the only signal separating a package that failed to build from one whose tests ran and failed. The name says what was observed and not what it means: such a package may have failed to build, may have had every test filtered out, or may have no tests, and runner output cannot tell those apart. Omitted when zero. **Action:** none. Read it as a reason to look, not as a diagnosis.

  Two limits worth knowing. The count originates only in a run that reads `--go-test` output and names a stream: a JUnit report carries no package-level event to count, so a JUnit-only adopter never sees the field. And `ingest --merge` carries a count forward from its inputs even though a merge names no stream of its own.

- **`streams` blocks are checked for consistency, and an inconsistent one exits `20`.** `coverage` and `sync` both refuse a results file whose `streams` block contradicts the entries beside it. Five rules: every stream an entry names must be declared, a stream name must not be empty and a non-empty one must not repeat, no count may be negative, and a stream's `extracted` must not be lower than the number of entries carrying its label. Higher is fine, because collapsing within a stream and merging across inputs both leave extraction legitimately above surviving entries. Code `20` is the first allocation from the evidence-stream band. **Action:** if you write results files by hand or with your own tool, an inconsistent block now fails a build that passed. Run `coverage --json` for the list.

  `ingest --merge` can produce one too. Merging a file that declares its streams with one carrying labeled entries and no block yields an artifact declaring the first stream and not the second, which `coverage` then refuses. C-44 exempts `--merge` deliberately, because it reads a file as a producer rather than as a consumer judging a workspace, but the exemption leaves Specter writing something Specter refuses. Tracked as `bugs/SP-SP-075`.

  `default` never requires declaration, whether an entry reaches it by carrying no label or by naming it outright. That exemption is what lets a project adopt `--stream` one job at a time instead of across every job at once.

  **A file with no `streams` key is untouched.** A file carrying `streams: []` or `streams: null` is not that file: the key is present, so a producer made a claim about what ran, and both are checked. **Action:** if a tool of yours emits an empty or null block beside labeled entries, either drop the key or declare the streams.

  The refusal appears in `coverage --json` as a `results_validation_errors` array. Each element carries `kind`, `stream`, `message`, and exactly one coordinate: `result_index` for `undeclared_stream`, which is about an entry, and `stream_index` for every other kind, which is about a declared row. Indexes point into the file as written rather than into the sorted array. A refusal still emits the whole document, so nothing that parsed the output before stops parsing it.
...
- **The coverage summary header reported an average that was one low.** The header sums each spec's percentage as a float and then floors, so a workspace with one spec at 100 percent and one at 33 reported 66 rather than 67. The number an operator reads on every run was wrong by one wherever the mean landed just under an integer. **Action:** none, unless you assert on the header text in a test. The figure may rise by one.
...
- **Two acceptance criteria sharing an id passed `check`, and coverage then reported 100 percent from a single annotation.** The id list is a slice while the annotation set is a set, so one annotation satisfied both entries. `check` now reports `duplicate_ac_id` at error severity, naming the id, how many times it is declared, and the positions. **Action:** a spec with a repeated criterion id now fails `check`. Its reported coverage was overstated before the fix, so the number may drop once the ids are made unique.

- **`structural_conflict` fired on sentences that merely mentioned the subject and an absence word.** Detection asked whether the required subject appeared anywhere in a downstream criterion and whether a word like `absent` or `without` appeared anywhere in the same criterion, with nothing binding the two. On Specter's own specs that produced 34 fires across every ordered pair, not one of them a genuine conflict, including `ci` matching inside `producing`. Detection now requires the absence expression to predicate the subject, and matches on whole words. The same corpus produces zero. **Action:** none. A diagnostic you were ignoring may stop appearing. **Note:** this does not make the check sound, and it is still advisory for that reason. It cannot tell a contradiction from a criterion testing the constraint being enforced, because `Process checkout when email is absent` and `Registration fails when email is absent` differ only in the outcome verb.
