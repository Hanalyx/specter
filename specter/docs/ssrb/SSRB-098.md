...
## 11. Addendum 2026-08-15: second project, and a defect in this brief's own reasoning

Status changed: **REJECT to NEEDS-DESIGN.** The original decision is not overturned.
Its stated basis no longer holds.

### The second project

Kensa filed `features/SP-KN-004-deferred-acceptance-criteria` on 2026-08-07,
asking for an AC that is known, agreed, and not yet implemented to be declarable
and excluded from the coverage denominator. JWTMS was the first requester in
April. This is the second, and the two are unrelated.

It does not cleanly trip the §8 trigger as worded. That trigger asks for a
product-state-only lifecycle explicitly orthogonal to coverage. Kensa's request
is coupled to coverage by design, because denominator exclusion is the point.

**The trigger was worded too narrowly.** It was written from the shape of the
first request rather than from the need underneath it. Read the two requests
together and the shared situation is not lifecycle labeling. It is a
specification written ahead of its implementation. JWTMS called that
`pending | draft`. Kensa calls it `deferred`. Neither is orthogonal to coverage,
because the whole difficulty is what coverage should do about it. A trigger that
can only fire for an orthogonal request could never have fired for this need.

### A defect in this brief's reasoning

Section 5 and §10 both reject the request by decomposition: each concern the
requester raised already has a home, and implementation gap has `gap: true`.

Verified 2026-08-15 against v0.14.1: **`gap` is inert.** It is written by the
reverse compiler and read by no stage of the pipeline. It does not leave the
denominator, it is not reported, it is not counted, and no gate consults it. It
records a fact and nothing acts on it.

So the decomposition assigns implementation gap to a slot that is empty. The
argument was sound in shape and false in one of its premises, and that premise is
load-bearing: remove it and there is no mechanism in the toolchain for declaring
that an acceptance criterion is deliberately not yet built.

### The mission-level argument neither requester made

Kensa measured the consequence: a spec can only be added after the code exists,
because a required 100% gate fails on any criterion without a passing test.

That inverts the methodology. Specter's stated order is spec first, code second.
Its gate enforces the opposite for any gap that outlives a single commit
sequence. The repository's own SDD cycle acknowledges the state and calls it
transient. Two adopters have now reported it as durable.

This is not a convenience request. It is a contradiction between what the tool
gates and what the tool is for, which is grounds for a design call regardless of
how many projects have asked.

### What must not be lost

The coverage implementation carries a deliberate guard: every declared AC counts
including `gap: true`, because otherwise a spec that is entirely gaps passes with
zero captured intent. Kensa's own request names the same hazard and forbids it.
Any design must keep a deferred criterion countable, visible on every run,
diffable when a criterion regresses into that state, and bounded. A mechanism
that lets a project quiet CI is worse than the present gap.

Note also that the Kensa request states an acceptance criterion carries exactly
four fields. It defines eleven. The requester measured their own corpus's usage,
not the schema, and future readers should not inherit that error.

### Disposition

The question is re-posed, not answered here, and it is narrower than #98's enum:
how does a project declare intent ahead of implementation without either failing
the gate or silencing it? That deserves its own brief rather than a third
addendum to a rejected one. This entry stands as the record that the rejection's
basis was checked and found wanting.
