...
The `strictness` setting was introduced in v0.11, but through v0.12 it was only partially enforced. `coverage` honored it for the zero-tolerance exit codes and the explicit `--strict` flag enforced outcome coverage, while the `sync` command quietly ignored the setting altogether — `sync`'s coverage phase always used the structural path, regardless of your `strictness` value. (Full routing of `--strictness threshold` through the strict report path in `coverage` itself landed with spec-coverage 1.15.0.) So on v0.12:
