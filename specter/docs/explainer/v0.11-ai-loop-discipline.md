...
`specter sync --strict` routes the check through automatically, so CI catches broken annotations before merge. The flag is what does it, not the manifest: `settings.strict: true` promotes warning and info diagnostics to error and does not run this scan. It used to, and that was the defect. A severity setting deciding which defects are discovered meant removing one manifest key made a broken annotation invisible, so `settings.strict` now governs severity alone. `spec-check` C-07 states what it does; C-09 names what enables the scan.
...
The hook delegates to a hidden `specter pre-push-check` subcommand that reads git's pre-push stdin format, runs `git diff` for each ref, and exits non-zero on impl-only diffs. Doc / spec / test changes always pass. A modified Go file counts only when it differs from its pushed base after canonical formatting, so a `goimports` regrouping or a `gofmt` reflow is not an implementation change. TypeScript, JavaScript, Python, Rust, Java, C, and C++ files always count, and so does any file the hook cannot read, or any Go file it cannot parse. Diffs that touch impl AND add an annotation delta (in any test file) pass.
...
The run now fails with an error naming the unknown key, suggesting the closest
valid one, and listing every key `settings:` accepts. The output is not quoted
here: it contains an em dash, and this document's style gate forbids one, so a
quoted copy would have to be altered and would stop being the real bytes. Run
`specter check` against a manifest with a bad key to see it.
...
It warns that no test file carried a `@spec`/`@ac` annotation and that coverage
will therefore report 0% for every spec, then names the two remedies: set
`settings.tests_glob` in `specter.yaml`, or pass `--tests <glob>`. As above, the
literal text is not reproduced because it carries an em dash this document may
not contain.
...
v0.10 closed the eval phase. v0.11 closes the order-of-operations phase. The remaining gaps are ergonomic, not structural. What is left for v0.12 is making each language and tool feel native rather than reaching past one more conceptual hole.
