...
# Emit the Markdown report
{
    ...
    echo
    echo "**A zero in the struct conflicts column usually means the check did"
    echo "not run.** \`check\` refuses structural checks while \`resolve\` has"
    echo "errors, so every repo with a non-zero resolve rc reports zero"
    echo "structural conflicts because none were looked for. Read that column"
    echo "only on the rows where resolve is 0."
    echo
    echo "**Structural conflicts are the number \`bugs/SP-SP-004\` needs**, and"
    echo "this run can supply it only for the repos that clear resolve. That"
    echo "defect is P0 and its false-positive rate has only ever been measured"
    echo "against the corpus the checker authors wrote themselves. Specs from"
    echo "code they have never seen are the measurement that matters, and"
    echo "SP-SP-060 is currently blocking it on ten of twelve repos."
    echo
    echo "**Orphan constraints are what \`check\` finds when it does run.**"
    echo "\`reverse\` writes constraints that no acceptance criterion references,"
    echo "and \`check\` treats that as an error. Both repos that cleared resolve"
    echo "failed on it, and every diagnostic in both runs is that one kind."
    echo "The reverse compiler is producing specs its own checker rejects,"
    echo "independently of the id collisions. Filed as \`bugs/SP-SP-062\`:"
    echo "\`reverse\` stamps \`enforcement: error\` on each generated"
    echo "constraint, which overrides the tier-3 \`info\` default that would"
    echo "otherwise make these advisory."
    echo
